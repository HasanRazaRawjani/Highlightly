...
Drop a clip — .mp4, .mov, or .mkv from any clipping software.



Highlightly edits locally — rendering runs on your PC, not in the cloud.



Get a vertical short — captions, optional background music, kill effects, and a file in edited_clips/ ready to post.
...
Try it (recommended for reviewers)
...
Live website — sign up and download the Windows installer
...
Click Download the app and run highlightly-setup.exe



Launch Highlightly, drop a Valorant clip into the queue, and click render
...
Python 3, Tkinter, Pygame (preview)
...
Groq API (Whisper Large V3), OpenAI Whisper (local fallback)
...
Caption font size / margin / zoom sliders
...
Kill FX (flash, shake, zoom, counter)
...
Add screenshots here after pushing to GitHub — e.g. docs/screenshots/app-queue.png, docs/screenshots/output-short.png.
